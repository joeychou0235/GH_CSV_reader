...
search_path = Path('/Users/joeychou/Documents/OneDrive - ジェネシスヘルスケア株式会社/ドキュメント/Ganesha_web02_backup/over')
...
with open('/Users/joeychou/Downloads/missing_samples2.txt') as source_file:
    ...
    for kit_code in source_reader:
        ...
        for textFileObj in search_path.glob('*CallRate*.csv'):
        ...
        for textFileObj in search_path.glob(filename[0:4]+'*Genotype.csv'):
            if os.stat(textFileObj).st_size != 0:
                with open(textFileObj) as t:
                    reader = csv.DictReader(t)
        ...
        for textFileObj in search_path.glob(filename[0:4]+'*Phenotype.csv'):
            if os.stat(textFileObj).st_size != 0:
                with open(textFileObj) as t:
                    reader = csv.DictReader(t)
...
write_into_csv('/Users/joeychou/Downloads/4missing_CallRate.csv', ['SampleCode','ChipCallrate','ProductCallrate','Status','StatusID'], callrate_data)
write_into_csv('/Users/joeychou/Downloads/4missing_Genotype.csv', ['kit_id','NEWGHnumber','allele'], genotype_data)
write_into_csv('/Users/joeychou/Downloads/4missing_Phenotype.csv', ['kit_id','unit_id','level_id','population_id','score','odds'], phenotype_data)
